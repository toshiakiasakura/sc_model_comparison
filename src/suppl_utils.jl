...
function clean_df_res_dataframe(df_sum::DataFrame)
	...
	df_res[!, :pc_error] = abs.(df_res.mean .- df_res.mean_raw) ./ df_res.mean_raw .* 100
	df_res[!, :pc_error_pos_neg] = (df_res.mean .- df_res.mean_raw) ./ df_res.mean_raw .* 100
...
function plot_validation_sample_vs_estimated(df_pos::DataFrame;
	...
	for (key, gdf) in zip(keys(gdfs), gdfs)
		...
		plot!(pl, [0, m_max], [0, m_max], color = :black, label = "", title = key[strat])
    ...
    plot!(pls[1], left_margin=10Plots.mm)
