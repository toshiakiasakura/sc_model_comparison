function summarise_data(df_dds::DataFrame; dir_ = "../dt_intermediate")
	df_sum = DataFrame()
	keys = df_dds[:, :key] |> unique
	for k in keys
		println(k)
		df_sum = vcat(df_sum, create_summary_stat_one_data(k; dir_=dir_))
	end
	CSV.write("$(dir_)/fit_summary.csv", df_sum)
end

function read_master_with_fit_summary(; update_data = false)
	df_dds = CSV.read("../dt_surveys_master/master_dds.csv", DataFrame);
	if update_data == true
		summarise_data(df_dds)
	end
